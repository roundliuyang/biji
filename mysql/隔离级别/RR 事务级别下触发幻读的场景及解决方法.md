...
>InnoDB 默认隔离级别是 `REPEATABLE READ`；它通过 **MVCC（快照读）+ 间隙锁（next-key lock）** 实现防止幻读；

`可重复读` 存在 `幻读`，但也可以 `避免` 幻读。

支持 `行锁`+`间隙锁(gap)`+`next-key lock(行锁+间隙锁)`+`表锁`。

正是因为本级别开始支持 `next-key lock` 机制，才有了解决幻读的可能。`行锁` 锁定等值记录，阻塞他事务的 `即时读` 操作，`间隙锁` 根据不同的查询条件，锁住与 `行` 相关的一个 `索引区间`，阻塞他事务对此 `索引区间` 做 **即时读** 操作（比如最直接引发幻读的 `insert` 操作）。
...
>**step1：加锁读（当前读）**
>
>- `FOR UPDATE` 是当前读，不走快照，会加锁；
>- 条件是 `id >= 1`，但由于表为空或从 `id >= 1` 开始查，所以 InnoDB 会加 **next-key lock**；
>- 加锁范围：`(1, +∞)` 或 `[1, +∞)`，甚至可能加上 **间隙锁 (gap lock)** 在索引里保护这个“空白区间”。
>
>**step2：插入被阻塞**
>
>- 由于事务A锁住了 `[1, +∞)` 的索引范围（间隙锁 + 行锁），
>- 所以事务B想插入 id=1、id=2，会触发 **间隙冲突**；
>- 插入被阻塞，必须等事务A释放锁。
>
>**step3:**
>
>- 第一个是 **快照读**：不加锁，读取的是旧版本；
>
>- 第二个是 **当前读**：再次加锁，结果不会变化，也不会新增幻行；
>
>- 所以无论是快照读还是当前读，**都不会出现幻读**，因为插入被阻塞了。
>
>  step4：提交，释放锁
>
>**step4：提交，释放锁**
...
![1690088648716](RR 事务级别下触发幻读的场景及解决方法.assets/1690088648716.png)

![1690088681185](RR 事务级别下触发幻读的场景及解决方法.assets/1690088681185.png)
...
![1690088844839](RR 事务级别下触发幻读的场景及解决方法.assets/1690088844839.png)

![1690088904939](RR 事务级别下触发幻读的场景及解决方法.assets/1690088904939.png)



此时命令行界面1执行**commit**,查询结果如下

![1690089031015](RR 事务级别下触发幻读的场景及解决方法.assets/1690089031015.png)
