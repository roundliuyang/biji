...
由于MySQL的InnoDB默认是使用的RR级别，所以我们先要将该**session**开启成RC级别，并且设置binlog的模式
...
![1706714891051](Innodb中的事务隔离级别和锁的关系.assets/1706714891051.png)

事务B修改id=1的数据提交之后，事务A同样的查询，后一次和前一次的结果不一样，这就是不可重读（重新读取产生的结果不一样）。这就很可能带来一些问题，**那么我们来看看在RR级别中MySQL的表现：**

![1706715404527](Innodb中的事务隔离级别和锁的关系.assets/1706715404527.png)
...
![innodb_lock_1](Innodb中的事务隔离级别和锁的关系.assets/6eb5d3b1.png)
...
![1706717671094](Innodb中的事务隔离级别和锁的关系.assets/1706717671094.png)

**RR级别：**

![1706717887686](Innodb中的事务隔离级别和锁的关系.assets/1706717887686.png)
...
![innodb_lock_2](Innodb中的事务隔离级别和锁的关系.assets/b3b6a55f.png)
...
![1706718713636](Innodb中的事务隔离级别和锁的关系.assets/1706718713636.png)
...
>Read Uncommitted 暂时这样理解吧
>
>1. **行级共享锁（读锁）：**
>   - 事务在读取数据时不会（不会主动）获取行级共享锁，因此不会阻塞其他事务的写操作。
>   - 读操作可以读取到其他事务尚未提交的数据。
>2. **行级排它锁（写锁）：**
>   - 事务在进行写操作时，会获取行级排它锁，**但这个锁在事务提交之前不会阻塞其他事务的读写操作。**



### Read Committed（读取提交内容）

1. 在MySQL的InnoDB存储引擎中，当事务在`READ COMMITTED`隔离级别下使用行级共享锁（也称为读锁）时，该锁在读取到数据后会立即释放。这种行为被称为“短暂读锁”或“瞬时读锁”。
2. 在MySQL的InnoDB存储引擎中，当事务在`READ COMMITTED`隔离级别下使用行级排它锁（也称为写锁）时，这些锁的释放方式有所不同。行级排它锁在`READ COMMITTED`级别下会在事务提交时释放，而不是在读取数据后立即释放。
...
- 当事务在`REPEATABLE READ`隔离级别下使用行级共享锁时，这些锁将会在事务结束时释放，而不是在读取数据后立即释放。这意味着在整个事务的生命周期中，对被锁定行的读锁将一直保持，直到事务结束。
- 这种方式确保了在`REPEATABLE READ`级别下，读取的数据在整个事务期间都保持一致。

**行级排它锁（写锁）：**

- 类似于`READ COMMITTED`级别，当事务在`REPEATABLE READ`级别下使用行级排它锁时，这些锁将在事务提交时释放。
- 行级排它锁在`REPEATABLE READ`级别下主要用于保护写操作，确保在整个事务过程中，其他事务无法获取对被锁定行的写权限。
...
1. **行级共享锁（读锁）：**
   - 当事务在`SERIALIZABLE`隔离级别下使用行级共享锁时，这些锁将会在事务结束时释放。类似于`REPEATABLE READ`级别，行级共享锁在整个事务期间保持，以确保读取的数据在整个事务过程中保持一致。
2. **行级排它锁（写锁）：**
   - 当事务在`SERIALIZABLE`级别下使用行级排它锁时，这些锁也将在事务结束时释放。行级排它锁主要用于保护写操作，确保在整个事务过程中，其他事务无法获取对被锁定行的写权限。

在`SERIALIZABLE`隔离级别下，所有事务都会相互等待，以确保没有并发操作。这种隔离级别提供了最高的数据一致性，但也可能导致性能问题，因为事务之间的竞争可能会导致较高的锁等待时间。
